...
SquareGraph::SquareGraph(int xLines, int yLines) : map(xLines, vector<Node>(yLines)), openNodes(), closedNodes() {}
	...
	while(!openNodes.empty()){
		...
		for(auto i = neighbours.begin(); i != neighbours.end(); ++i){
			...
			if(!(neighbourPtr->isClosed())){
				int tentativeScore = currentNode.getCostFromStart() 
					+ this->calculateDistance(make_pair(currentPtr->x, currentPtr->y), 
					make_pair(neighbourPtr->x, neighbourPtr->y));

				if( (!neighbourPtr->isOpen()) 
					|| (tentativeScore < currentNode.getCostFromStart())){
