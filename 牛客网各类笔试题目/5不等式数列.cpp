...
dp[i - 1][j - 1] * (i - j - 1) 将i加载任意一个大于号之间，等于加入了一个小于号；
即要求i - 1个数时有了j - 1个小于号，而此时共有(i - 1) - (j - 1) - 1个大于号，
每个大于号都要进行一次这样的操作
...
int dp[1001][1001]; //不能将此数组放在main函数内部
//因为内部属于局部变量，貌似堆栈容量不够的
//1001*1001*4/1024/=978.5KB
