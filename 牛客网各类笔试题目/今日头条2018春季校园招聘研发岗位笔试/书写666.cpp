...
这题其实网上找个字符串表达式求值的，
把表达式值给求出来， 然后就好办了。
今日头条6周年周年庆就要开始啦。活动主办方请你帮忙制作一个小彩蛋。你的程序需要读取一个表达式，
并输出用字符6拼出的计算结果。相邻数字使用两个英文句号"."间隔，如下是0123456789。
...
对于100%的数据，表达式仅包含'6', '+', '-', '*'四种字符。1≤n≤100，
表达式长度不超过100，其中'+', '-', '*'均为二元运算符，计算中间结果在[−263,263−1]之内，
最终结果在[0,263−1]之内
输出描述：
对于每组数据输出用字符6拼出的计算结果。
...
for i in range(t):
    ...
    for j in range(5):
        for k in range(len(ans)):
            if k == len(ans) - 1:
                print(num_str[int(ans[k])][j])
            else:
                print(num_str[int(ans[k])][j], end='..')